...
static void arrange_root(struct sway_root *root) {
	...
	for (int i = 0; i < root->unmapped_containers->length; ++i) {
		struct sway_container *container = root->unmapped_containers->items[i];
...
static void animate_root(struct sway_root *root) {
	...
	if (root->unmapped_containers->length > 0) {
		double t;
		...
		for (int i = 0; i < root->unmapped_containers->length; ++i) {
			...
			view_reconfigure(container->view);
		}
	}
	arrange_popups(root->layers.popup);
}
...
static void save_animation_variables() {
	...
	if (!fs) {
		for (int j = 0; j < root->unmapped_containers->length; ++j) {
			struct sway_container *container = root->unmapped_containers->items[j];
